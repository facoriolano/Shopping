...
Sua resposta DEVE ser APENAS um único bloco de código JSON válido, sem nenhum texto, markdown (como \`\`\`json) ou explicação adicional. Siga estritamente este formato:
{
  ...
  const ProductCard: React.FC<{ product: Product, index: number }> = ({ product, index }) => {
    ...
    const PriceChangeIndicator: React.FC<{ history: PriceHistoryEntry[] }> = ({ history }) => {
        ...
        return (
            <div className={`flex items-center text-xs font-semibold ${isDrop ? 'text-green-400' : 'text-red-400'}`}>
                {isDrop ? (
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden transition-all duration-300 flex flex-col group relative">
          ...
          {isAdminMode && (
              <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button onClick={() => setEditingState({ product, index, newUrl: product.originalUrl, newAffiliateIds: { amazon: product.affiliateIds?.amazon || '', mercadoLivre: product.affiliateIds?.mercadoLivre || '' } })} className="bg-gray-700/60 hover:bg-cyan-600/90 text-white p-2 rounded-full" aria-label="Editar produto" disabled={isLoading || isUpdatingAll || isSavingEdit || updatingProductIndex !== null}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fillRule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clipRule="evenodd" /></svg></button>
                <button onClick={() => handleDeleteProduct(index)} className="bg-gray-700/60 hover:bg-red-600/90 text-white p-2 rounded-full" aria-label="Excluir produto" disabled={isLoading || isUpdatingAll || isSavingEdit || updatingProductIndex !== null}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" clipRule="evenodd" /></svg></button>
              </div>
          )}
            ...
            <div className="bg-gray-700/50 p-4 border-t border-gray-700">
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData} margin={{ top: 5, right: 20, left: -15, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#556278" />
                  <XAxis dataKey="formattedDate" stroke="#a0aec0" fontSize={12} />
                  <YAxis stroke="#a0aec0" fontSize={12} tickFormatter={(value) => `R$${value}`} domain={[(dataMin: number) => (dataMin > 20 ? dataMin - 20 : 0), (dataMax: number) => dataMax + 20]} />
                  <Tooltip
        ...
        <div className="max-w-7xl mx-auto">
          <header className="text-center mb-8 relative">
            ...
            <div className="absolute top-0 right-0 hidden sm:block">
                <label htmlFor="admin-toggle" className="flex items-center cursor-pointer"><span className="mr-3 text-sm font-medium text-gray-300">Modo Loja</span><div className="relative"><input type="checkbox" id="admin-toggle" className="sr-only" checked={isAdminMode} onChange={() => setIsAdminMode(!isAdminMode)} /><div className="block bg-gray-600 w-14 h-8 rounded-full"></div><div className="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform"></div></div><span className="ml-3 text-sm font-medium text-cyan-400">Modo Admin</span></label>
            </div>
            <style>{`input:checked ~ .dot { transform: translateX(100%); background-color: #0891b2; }`}</style>
          </header>

          <main>
            {isAdminMode && (
                <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8 sticky top-4 z-30">
                <h2 className="text-2xl font-semibold mb-4 text-white">Adicionar Novo Produto</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <input type="text" value={globalAffiliateIds.amazon} onChange={(e) => setGlobalAffiliateIds(prev => ({...prev, amazon: e.target.value}))} placeholder="Seu ID de Afiliado Amazon (tag)" className="bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-3 px-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" aria-label="Seu ID de Afiliado Amazon (Global)" />
                    <input type="text" value={globalAffiliateIds.mercadoLivre} onChange={(e) => setGlobalAffiliateIds(prev => ({...prev, mercadoLivre: e.target.value}))} placeholder="Seu ID de Afiliado Mercado Livre (afid)" className="bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-3 px-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" aria-label="Seu ID de Afiliado Mercado Livre (Global)" />
                </div>
                <div className="flex flex-col sm:flex-row gap-4">
                    <input type="url" value={productUrl} onChange={(e) => setProductUrl(e.target.value)} placeholder="Cole o link do produto aqui (Ex: Amazon)" className="flex-grow bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-3 px-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" disabled={isLoading || isUpdatingAll || updatingProductIndex !== null} aria-label="URL do Produto" />
                    <button onClick={handleAddProduct} disabled={isLoading || isUpdatingAll || updatingProductIndex !== null || !productUrl} className="bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-3 px-6 rounded-md transition-colors duration-300 flex items-center justify-center">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              ...
              {isAdminMode && (<button onClick={handleUpdateAllProducts} disabled={isLoading || isUpdatingAll || updatingProductIndex !== null || products.length === 0} className="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-md transition-colors duration-300 flex items-center justify-center gap-2" aria-label="Atualizar preços de todos os produtos">
            ...
            <div className="bg-gray-800 p-4 rounded-lg shadow-lg mb-6 flex flex-col sm:flex-row justify-center items-center gap-6">
                <div className="flex items-center gap-2"><label htmlFor="filter-select" className="text-gray-300 font-medium text-sm">Filtrar:</label><select id="filter-select" value={filter} onChange={e => setFilter(e.target.value)} className="bg-gray-700 text-white border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500"><option value="all">Todos os produtos</option><option value="onSale24h">Em promoção (24h)</option></select></div>
                <div className="flex items-center gap-2"><label htmlFor="sort-select" className="text-gray-300 font-medium text-sm">Ordenar:</label><select id="sort-select" value={sort} onChange={e => setSort(e.target.value)} className="bg-gray-700 text-white border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500"><option value="default">Padrão</option><option value="priceAsc">Menor Preço</option><option value="priceDesc">Maior Preço</option></select></div>
            </div>

            {displayedProducts.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {displayedProducts.map((product, index) => (<ProductCard key={`${product.originalUrl}-${index}`} product={product} index={products.findIndex(p => p.originalUrl === product.originalUrl)} />))}
                </div>
            ) : (
                <div className="text-center py-16 px-6 bg-gray-800 rounded-lg">
                    <h3 className="text-2xl font-bold text-gray-300">Nenhum produto encontrado</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4" aria-modal="true" role="dialog">
          <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
            ...
            <div className="space-y-4">
              <div><label htmlFor="edit-url" className="block text-sm font-medium text-gray-300 mb-1">URL do Produto</label><input type="url" id="edit-url" value={editingState.newUrl} onChange={(e) => setEditingState({ ...editingState, newUrl: e.target.value })} className="w-full bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500" /></div>
              <div><label htmlFor="edit-affiliate-id-amazon" className="block text-sm font-medium text-gray-300 mb-1">ID de Afiliado Amazon</label><input type="text" id="edit-affiliate-id-amazon" value={editingState.newAffiliateIds.amazon} onChange={(e) => setEditingState(prev => prev ? { ...prev, newAffiliateIds: {...prev.newAffiliateIds, amazon: e.target.value } } : null)} placeholder="Deixe em branco para usar o ID global" className="w-full bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500" /></div>
              <div><label htmlFor="edit-affiliate-id-ml" className="block text-sm font-medium text-gray-300 mb-1">ID de Afiliado Mercado Livre</label><input type="text" id="edit-affiliate-id-ml" value={editingState.newAffiliateIds.mercadoLivre} onChange={(e) => setEditingState(prev => prev ? { ...prev, newAffiliateIds: {...prev.newAffiliateIds, mercadoLivre: e.target.value } } : null)} placeholder="Deixe em branco para usar o ID global" className="w-full bg-gray-700 text-white placeholder-gray-400 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500" /></div>
            </div>
            {error && editingState && <p className="text-red-400 mt-4 text-center">{error}</p>}
            <div className="mt-6 flex justify-end gap-4">
              <button onClick={() => { setEditingState(null); setError(null); }} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-md transition-colors" disabled={isSavingEdit}>Cancelar</button>
              <button onClick={handleSaveEdit} className="bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-md transition-colors flex items-center justify-center min-w-[110px]" disabled={isSavingEdit}>
